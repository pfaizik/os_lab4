...
enum HTTP_REQUEST_TYPE http_parse_request (char *request) {
 ...
 if (strncmp(request, http_index, strlen(http_index)) == 0) return HTTP_REQ_INDEX_HTML;
 if (strncmp(request, http_readdata, strlen(http_readdata)) == 0) return HTTP_REQ_READDATA;
 if (strncmp(request, http_writedata, strlen(http_writedata)) == 0) return HTTP_REQ_WRITEDATA;
